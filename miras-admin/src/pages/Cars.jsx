    ...
    <div className="space-y-5 page-enter">
      ...
      {loading ? (
        ...
        <div className="card p-16 text-center">
          ...
          <p className="text-gray-800 font-semibold mb-1">No cars found</p>
          <p className="text-gray-400 text-sm mb-4">Add your first car to get started</p>
          <Link to="/cars/add" className="btn-primary inline-flex items-center gap-2">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-black/5 rounded-2xl p-6 max-w-sm w-full shadow-lg">
            <h3 className="font-bold text-gray-800 text-lg mb-2">Remove Car?</h3>
...
function CarCard({ car, onDelete }) {
  ...
  return (
    <div className="card overflow-hidden group hover:border-primary/30 transition-all duration-300">
      {/* Image */}
      <div className="h-44 bg-gray-100 relative overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="badge bg-white/90 text-gray-600 backdrop-blur-sm">{car.category || 'SUV'}</span>
        </div>
      </div>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div>
            <h3 className="font-bold text-gray-800">{car.name || 'Unnamed Car'}</h3>
            <p className="text-sm text-gray-400">{car.modelYear || '—'} · {car.transmission || 'Manual'}</p>
          </div>
          <div className="text-right">
            <p className="text-primary font-bold text-lg">₹{car.pricePerDay?.toLocaleString() || '—'}</p>
        ...
        {car.bookedUntil && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5 mb-3 text-xs text-amber-600 flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-2 pt-1">
          <Link to={`/cars/edit/${car._id}`} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-gray-100 hover:bg-primary/10 hover:text-primary text-gray-500 text-sm transition-all">
            <Edit2 size={13} /> Edit
          </Link>
          <button
