    ...
    <div className="max-w-3xl mx-auto space-y-6 page-enter">
      ...
      <div className="card p-6">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-9 h-9 rounded-xl bg-brand-gold/10 flex items-center justify-center"><Globe size={17} className="text-brand-gold" /></div>
          <h2 className="font-display font-bold text-white text-lg">Business Information</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-600 mt-1">Used for customer notification links. Format: country code + number (e.g. 919876543210)</p>
          </div>
          <div>
        ...
        <button onClick={saveBusiness} disabled={saving} className="btn-gold mt-5 flex items-center gap-2">
          {saving ? <><div className="w-4 h-4 border-2 border-dark-900/30 border-t-dark-900 rounded-full animate-spin"/>Saving…</> : <><Save size={15}/>Save Changes</>}
        </button>
      </div>
      ...
      <div className="card p-6">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-9 h-9 rounded-xl bg-blue-500/10 flex items-center justify-center"><Bell size={17} className="text-blue-400" /></div>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={item.key} className="flex items-center justify-between p-4 bg-dark-700/40 rounded-xl">
              <div>
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full transition-all flex-shrink-0 relative ${notifs[item.key] ? 'bg-brand-gold' : 'bg-dark-500'}`}
              >
      ...
      <div className="card p-6">
        <div className="flex items-center gap-3 mb-5">
          <div className="w-9 h-9 rounded-xl bg-purple-500/10 flex items-center justify-center"><Shield size={17} className="text-purple-400" /></div>
      ...
      <div className="card p-6 flex items-center gap-4">
        <div className="w-14 h-14 rounded-2xl bg-gold-gradient flex items-center justify-center shadow-gold flex-shrink-0">
          <span className="text-dark-900 font-display font-bold text-2xl">M</span>
        </div>
        <div>
          <p className="font-display font-bold text-white text-lg">Miras Admin Panel</p>
          <p className="text-gray-500 text-sm">Version 1.0.0 · Built for Miras Car Rental, Kashmir</p>
          <p className="text-xs text-gray-600 mt-1">Backend: localhost:5000 · Frontend: localhost:3000 · Admin: localhost:3001</p>
        </div>
      </div>
    </div>
  )
