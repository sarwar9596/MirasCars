...
const CATEGORIES = ['All', 'SUV', 'Sedan', 'Hatchback', 'Luxury', 'MUV'];
		...
		<div style={{ background: '#F8F9FA' }}>
			...
			>
				<div className='relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
					<span className='section-label mb-3 block'>Choose Your Ride</span>
					<h1 className='font-display font-bold text-white mb-3' style={{ fontSize: 'clamp(2rem, 4vw, 3rem)' }}>
						Our Premium Fleet
					</h1>
					<p className='text-lg' style={{ color: 'rgba(255,255,255,0.8)' }}>
						From rugged SUVs to luxury sedans — pick the perfect car for your Kashmir trip
					</p>
				</div>
				{/* Wavy bottom */}
			...
			{/* ── Filters & Search ────────────────────── */}
			<section className='py-8 sticky top-[72px] z-40' style={{ background: 'rgba(248,249,250,0.85)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
						...
						<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
							{filteredCars.map((car) => (
								...
								>
									...
									<div className='p-5'>
										<h3 className='text-base font-bold mb-1' style={{ color: '#1A1A1A' }}>{car.name}</h3>

										{/* Specs */}
										<div className='grid grid-cols-2 gap-2 mb-4 text-xs' style={{ color: '#6B7280' }}>
											...
											{car.modelYear && <span className='flex items-center gap-1'><Gauge size={12} /> {car.modelYear}</span>}
										</div>
										...
										{car.features && car.features.length > 0 && (
											<div className='flex flex-wrap gap-1.5 mb-4'>
												{car.features.slice(0, 3).map((feature, i) => (
													<span key={i} className='px-2 py-0.5 text-xs rounded-full' style={{ background: 'rgba(47,164,169,0.08)', color: '#2FA4A9' }}>
														{feature}
													</span>
												))}
												{car.features.length > 3 && (
													<span className='text-xs' style={{ color: '#6B7280' }}>+{car.features.length - 3}</span>
												)}
											</div>
										)}
										...
										<div className='flex items-center justify-between pt-4' style={{ borderTop: '1px solid rgba(0,0,0,0.06)' }}>
											<div>
												<span
													...
													style={{ background: 'linear-gradient(135deg, #FF8A3D, #F2994A)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}
												>
													₹{car.pricePerDay?.toLocaleString()}
												</span>
												<span className='text-sm' style={{ color: '#6B7280' }}>/day</span>
											</div>
											<span className='text-sm font-semibold' style={{ color: '#FF8A3D' }}>View Details →</span>
										</div>
									</div>
								</Link>
							))}
